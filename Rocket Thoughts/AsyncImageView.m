...
@synthesize imgViewRect,isCroppedImage;
...
- (void)setImageURL:(NSURL *)imageURL
{
    NSString *strUrl = [imageURL absoluteString];
    if (strUrl != nil) {
...
{
    //This method is not used now, as we are setting the property of image view
    if (isCroppedImage) {
        CGRect cropRect;
        if (image.size.height<imgViewRect.size.height || image.size.width<imgViewRect.size.width) {
            image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(21.0, 13.0, 21.0, 13.0)];
        }
        cropRect.size.width =imgViewRect.size.width;
        cropRect.size.height =imgViewRect.size.height;
        cropRect.origin.x = image.size.width/2-imgViewRect.size.width/2-5;
        cropRect.origin.y = image.size.height/2-imgViewRect.size.height/2-5;
